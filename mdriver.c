					 ...
					 int tracenum, int opnum)
{
	...
	/* "페이로드는 힙의 범위 내에 있어야 합니다."

	여기서 "페이로드(payload)"는 주로 데이터를 의미하는 용어로 사용되며,
	"힙(heap)"은 메모리의 일부분을 가리키는데 동적 메모리 할당이 이루어지는 부분입니다.
	따라서, 이 문장은 동적으로 할당된 데이터가 힙 메모리의 영역 내에 있어야 한다는 것을 의미합니다. */
